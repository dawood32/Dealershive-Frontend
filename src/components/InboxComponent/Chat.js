...
import {blockuser} from '../../redux/action/chatapi';
var longitudes;
var latitudes;
var latitude;
var longitude;
...
const Chat = ({navigation, route}) => {
  ...
  const [searchmesg, setsearchmesg] = useState(false);
  ...
  const offsoket = async () => {
    let userid = await AsyncStorage.getItem('@user_id');

    socket.off('message' + userid);
  };
  const socketpros = msge => {
    comments.unshift(msge);
    console.log(comments, 'epsssssss');

    let k = {status: true, data: comments};
  ...
  const recentchatfunc = item => {
    if (item.status == 'send') {
      let params = 'id=' + route.params.Messagelist.to_user;
      dispatch(recentChat(params, onSuccess5));
    } else {
      let params = 'id=' + route.params.Messagelist.from_user;
      dispatch(recentChat(params, onSuccess5));
    }
  ...
  const userblocked = () => {
    if (userInfo.status == 'send') {
      let params = 'id=' + userInfo.to_user;
      dispatch(blockuser(params));
    } else {
      let params = 'id=' + userInfo.from_user;
      dispatch(blockuser(params));
    }
    setBlockChat(false);
  };
      ...
      {location ? (
        ...
        <View style={styles.container}>
                ...
                <TouchableOpacity style={styles.mainsearch}>
                  <ImageBackground
                    ...
                    <TextInput
                      ...
                      onSubmitEditing={() => searchresultfunc(userInfo)}
          ...
          <View style={styles.flatlistview}>
            ...
            <FlatList
              data={recentchatdata}
              inverted={searchmesg ? false : true}
                ...
                return (
                  <View>
                    {item.status == 'recive' ? (
                      item.type == 'text' ? (
                        <View
                          style={[styles.otherview, {flexDirection: 'row'}]}>
                          <View
                            style={{
                              width: '10%',
                              height: 40,
                          ...
                          <View style={[styles.category1, {left: 10}]}>
                            <View style={{width: '76%'}}>
                        ...
                        <View
                          ...
                          <TouchableOpacity
                            ...
                            style={[styles.category4, {left: 10}]}>
                            <Video
                              source={{
                                uri: item.message,
                              }}
                        ...
                        <View
                          ...
                          <View
                            style={[
                              styles.category5,
                              {backgroundColor: '#F1F5F9', left: 10},
                            ]}>
                            <TouchableOpacity
            ...
            <View style={styles.bottomview}>
              <View style={styles.bottom}>
                ...
                <View
                  ...
                  <Menu>
                    <MenuTrigger onPress={() => setErrorMessage(false)}>
                      {/* <View style={{width: 30}}> */}
                      <Image
                    ...
                    <MenuOptions
                      ...
                      <View style={styles.menuoptionview}>
                        ...
                        <MenuOption onSelect={() => requestCameraPermission()}>
                          <View>
                            <Image
                  ...
                  <TouchableOpacity onPress={() => permissionAudio()}>
                    <Image
